...
bout="${build}/out/target/product/falcon"
...
# Upload md5sum
ncftpput -v -u "${buser}" -p "${bpass}" ${bserver} /falcon/F-AOSP/5.1 ${bout}/AOSP*.md5sum
...
ncftpput -v -u "${buser}" -p "${bpass}" ${bserver} /falcon/F-AOSP/5.1 ${bout}/AOSP*.zip
